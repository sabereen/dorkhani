...
ساخت، مشاهده و مشارکت همچنان برای مهمان قابل استفاده‌اند. Basic Auth زیرمسیر مدیریت مستقل است و از
نشست کاربران استفاده نمی‌کند.
...
`TKhatm.ownerId` مالک اختیاری ختم است و تمام actionهای ویرایش و حذف، مالکیت را دوباره در service
سرور کنترل می‌کنند. `ownerId` و `guestClaimTokenHash` با نوع `KhatmData` از DTOهای مرورگر حذف
می‌شوند. صفحه `/account` آخرین دور هر دنباله را نمایش می‌دهد و صفحه
...
حذف مالکانه، تمام دورها و `TKhatmPart`های وابسته را در یک تراکنش پاک می‌کند. جدول
`TKhatmDeletion` فقط `khatmId`، `seriesId` و `deletedAt` را به‌عنوان tombstone نگه می‌دارد تا لینک
حذف‌شده پاسخ `410 Gone` بگیرد، در حالی که شناسه‌ای که هیچ‌گاه وجود نداشته همچنان `404` است.
