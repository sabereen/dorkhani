...
ایتا endpoint برابر `/api/auth/sign-in/eitaa` را فراهم می‌کند و plugin بله endpoint متناظر
`/api/auth/sign-in/bale` را در اختیار می‌گذارد. هر دو `initData` را با HMAC-SHA256 و مقایسه
ثابت‌زمان اعتبارسنجی می‌کنند؛ مهلت ایتا ۶۰ ثانیه و مهلت بله ۵ دقیقه است. مینی‌اپ بله در layout ریشه
ورود را خودکار آغاز می‌کند. اگر نشست فعال با هویت بله متفاوت باشد، کاربر بین حفظ نشست، اتصال هویت
متصل‌نشده به نشست فعلی یا جابه‌جایی به حساب بله انتخاب می‌کند و هیچ ادغام خودکاری رخ نمی‌دهد.
...
`GOOGLE_CLIENT_ID`، `GOOGLE_CLIENT_SECRET`، `EITAA_APP_TOKEN`، `BALE_BOT_TOKEN`،
`EITAA_BOT_USERNAME`، `BALE_BOT_USERNAME`، `BALE_WEBHOOK_SECRET` و تنظیمات `SMTP_*`.

## ۱۸. اعلان‌های کاربری چندکاناله

`NotificationPreference` روشن یا خاموش بودن اعلان، فعال بودن هر کانال و کانال ترجیحی را نگه
می‌دارد. `NotificationEndpoint` مقصد و مجوز ارسال خصوصی بله یا ایتا را بر اساس user ثبت می‌کند؛
ایمیل از نشانی تأییدشدهٔ `User` به دست می‌آید. نبود preference به معنی تنظیم پیش‌فرض فعال با ترتیب
`bale`، سپس `eitaa` و سپس `email` است.

رویدادهای ساخت ختم، انتخاب بازه یا آیات یا ذکر، تکمیل دور و پایان دنباله پس از commit عملیات اصلی
به `user-notification.ts` فرستاده می‌شوند. ارسال best-effort و بدون outbox است: نخست کانال ترجیحی و
سپس fallbackها امتحان می‌شوند، خطای provider log می‌شود و هرگز عملیات ختم را rollback نمی‌کند.
گذار اتمیک `inProgress` به `completed` مانع اعلان تکراری پایان یک دور می‌شود.
