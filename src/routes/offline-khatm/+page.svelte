...
{#if !checked}
	<div class="offline-loading" role="status">
		<span class="ui-spinner"></span><span>{m.offline_preparing()}</span>
	</div>
{:else if !supported}
...
{:else if loadError && !selected}
	<section class="ui-alert ui-alert-error" role="alert">
		<div>
			<strong>{m.offline_failure_title()}</strong>
			<p>{loadError}</p>
		</div>
		<button
			class="ui-btn ui-btn-outline"
			type="button"
			onclick={() => (selectedId ? loadSelected() : loadList())}>{m.common_retry()}</button
		>
	</section>
...
{:else if selected}
	<OfflineKhatmDetail
		...
		onRead={(selectedRange) =>
			navigate({ id: selected!.id, view: 'read', range: selectedRange.toRangeParam() })}
	...
	<div class="offline-dashboard">
			...
			<section class="ui-card ui-card-bordered offline-list-card">
				<div class="ui-card-body">
					<header class="offline-section-heading">
						<div>
							<h2>{m.offline_list_title()}</h2>
							<p>{m.offline_list_description()}</p>
						</div>
									...
									<span class="offline-list-copy">
										...
										<small
											>{khatm.status === 'completed'
												? m.offline_completed()
												: m.offline_progress({
														percent: khatm.pageProgress.toLocaleString(),
													})}</small
										>
									</span>
									{#if khatm.series}<span class="ui-badge ui-badge-accent"
											><IconRepeat />{m.offline_round({
												count: khatm.roundNumber.toLocaleString(),
											})}</span
										>{/if}
		...
		{:else if !loading}
			<div class="offline-empty">
				<IconBook />
				<h2>{m.offline_empty_title()}</h2>
				<p>{m.offline_empty_description()}</p>
			</div>
		{/if}
...
<style>
	...
	.offline-unsupported h2 {
		margin: 1rem 0 0;
	}
	.offline-unsupported p {
		max-width: 34rem;
	...
	.offline-intro .offline-main-icon {
		margin: 0;
	}
	.offline-intro h2,
	.offline-intro p,
	...
	.offline-intro > div > p:last-child,
	.offline-section-heading p {
		margin-top: 0.25rem;
		color: var(--ui-color-text-muted);
		line-height: 1.8;
	...
	.offline-khatm-list li + li {
		border-top: 1px solid var(--ui-color-border);
	}
	.offline-khatm-list a {
		display: flex;
		align-items: center;
		padding: 0.8rem;
	...
	.offline-list-copy strong,
	.offline-list-copy small {
		display: block;
	}
	.offline-list-copy small {
		margin-top: 0.15rem;
